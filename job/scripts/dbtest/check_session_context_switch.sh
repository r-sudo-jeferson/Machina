...
readonly WORKSPACE_B_SECOND='20000000-0000-0000-0000-0000000000b3'
...
VALUES ('${TENANT_B}', '${WORKSPACE_B_SECOND}', 'secondary', 'Secondary');
...
switch_result="$(query_as "$RUNTIME_ROLE" "SELECT session_id::text || ':' || active_tenant_id::text || ':' || active_workspace_id::text || ':' || extract(epoch FROM expires_at)::bigint::text FROM iam.switch_session_context(decode('${SWITCH_OLD_HASH}', 'hex'), decode('${SWITCH_NEW_HASH}', 'hex'), decode('${SWITCH_NEW_CSRF_HASH}', 'hex'), '${TENANT_B}')")"
expect_equals "${SWITCH_SESSION_ID}:${TENANT_B}:${WORKSPACE_B}:${switch_expiry_before}" "$switch_result" 'tenant switch did not select the authoritative first workspace and preserve expiry'
...
expect_equals "${TENANT_B}:${WORKSPACE_B}:${switch_expiry_before}" "$new_context" 'rotated session does not carry server-selected context with unchanged absolute expiry'
...
WHERE tenant_id = '${TENANT_B}' AND subject_id = '${SUBJECT_A}';
SQL
expect_failure "$RUNTIME_ROLE" "SELECT * FROM iam.switch_session_context(decode('${REVOKED_OLD_HASH}', 'hex'), decode('${REVOKED_NEW_HASH}', 'hex'), decode('${REVOKED_NEW_CSRF_HASH}', 'hex'), '${TENANT_B}')" 'revoked membership selected a tenant'
...
WHERE id = '${TENANT_B}';
SQL
expect_failure "$RUNTIME_ROLE" "SELECT * FROM iam.switch_session_context(decode('${SUSPENDED_OLD_HASH}', 'hex'), decode('${SUSPENDED_NEW_HASH}', 'hex'), decode('${SUSPENDED_NEW_CSRF_HASH}', 'hex'), '${TENANT_B}')" 'suspended tenant became active session context'
...
expect_failure "$RUNTIME_ROLE" "SELECT * FROM iam.switch_session_context(decode('${EXPIRED_OLD_HASH}', 'hex'), decode('${EXPIRED_NEW_HASH}', 'hex'), decode('${EXPIRED_NEW_CSRF_HASH}', 'hex'), '${TENANT_B}')" 'expired session switched tenant context'

printf 'dbtest: atomic server-validated tenant switch boundary passed\n'
